...
using HamedStack.SystemTextJson.ConcreteTypes;
using HamedStack.SystemTextJson.Enums;
using HamedStack.SystemTextJson.JsonPath;

namespace HamedStack.SystemTextJson;

public static class JsonExtensions
{
    public static IEnumerable<JsonElement> JsonPath(this string json, string jsonPath)
    {
        ...
        return JsonSelector.Select(doc.RootElement, jsonPath);
    }

    public static string JsonPathAsString(this string json, string jsonPath, JsonSerializerOptions? options)
    {
        if (string.IsNullOrWhiteSpace(json))
            throw new ArgumentException("Value cannot be null or whitespace.", nameof(json));
        if (string.IsNullOrWhiteSpace(jsonPath))
        ...
        using var doc = JsonDocument.Parse(json);
        var selector = JsonSelector.Select(doc.RootElement, jsonPath);
        var options1 = options ?? new JsonSerializerOptions { WriteIndented = true };
        return JsonSerializer.Serialize(selector, options1);
    }

    public static IEnumerable<JsonElement> JsonPath<T>(this T obj, string jsonPath)
    {
    ...
    public static string JsonPathAsString<T>(this T obj, string jsonPath, JsonSerializerOptions? options)
    {
        if (obj == null) throw new ArgumentNullException(nameof(obj));
        if (string.IsNullOrWhiteSpace(jsonPath))
            throw new ArgumentException("Value cannot be null or whitespace.", nameof(jsonPath));
            ...
            switch (root1.ValueKind)
            {
                ...
                default:
                    var result = new JsonComparisonResult
                    {
                        ...
                        Status = root1.GetRawText() != root2.GetRawText() ? JsonComparisonStatus.Modified : JsonComparisonStatus.Unchanged
                    };
    ...
    {
        return JsonSerializer.Deserialize<T>(data.GetString(), options);
    }

    public static T? Deserialize<T>(this Stream stream, JsonSerializerOptions? options = null)
    {
        return JsonSerializer.Deserialize<T>(stream.GetString(), options);
    }
    ...
    {
        var jsonElement = jsonDocument.RootElement;
        return jsonElement.GetKeys(separator).Select(x => x[..x.LastIndexOfAny(new[] { '-' })]);
    }
    ...
    private static string GetString(this Stream @this)
    {
    ...
    private static string Trim(this string source, string trimChars)
    {
        return string.IsNullOrEmpty(trimChars) ? source.Trim() : source.Trim(trimChars.ToCharArray());
    }
}